...
router.post("/enter-fare-entry", async (req, res) => {
	try {
		...
		return res.status(201).json({ message: "Fare entry created successfully" });
	} catch (error) {
		console.error(error);
...
router.post("/addBusStop", async (req, res) => {
	try {
		...
		return res.status(201).json({ message: "Bus Stop created successfully" });
	} catch (error) {
		console.error(error);
		return res.status(500).json({ error: "Internal Server Error" });
	}
	...
	try {
		...
		if (existingRouteStop) {
			return res.status(400).json({
				error:
					"RouteStop entry already exists for the given route and bus stop",
			});
		}
		...
		return res.json({ message: "RouteStop entry added successfully" });
	} catch (error) {
		console.error(error);
		return res.status(500).json({ error: "Internal Server Error" });
	}
...
router.get(
	...
	async (req, res) => {
		try {
			...
			if (!startStop) {
				console.log("Start bus stop not found");
			...
			if (!endStop) {
				console.log("End bus stop not found");
			...
			if (!routeStopForStartStop) {
				console.log("RouteStop entry not found for startStop");
				return res
					.status(404)
					.json({ error: "RouteStop entry not found for startStop" });
			}
			...
			if (!routeStopForEndStop) {
				console.log("RouteStop entry not found for endStop");
				return res
					.status(404)
					.json({ error: "RouteStop entry not found for endStop" });
			}
			...
			const fareLevelDifference = Math.abs(
				startStopFareLevel - endStopFareLevel
			);

			console.log(`fareLevelDifference:`, fareLevelDifference);

			const fareLevel = await FareTable.findOne({
				fareLevel: fareLevelDifference,
			});
			if (!fareLevel) {
				console.log("Fare level difference not found");
				return res
					.status(404)
					.json({ error: "Fare level difference not found" });
			}

			const farePrice = fareLevel.farePrice.toFixed(2).toString();
			console.log(`farePrice:`, farePrice);
			return res.status(200).send(farePrice);
		} catch (error) {
			console.error(error);
			return res.status(500).json({ error: error.message });
		}
